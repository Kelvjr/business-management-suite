...
import { paymentState, roundMoney } from "../domain/money.js";
import { stockAfterMovement, stockDeltas } from "../domain/inventory.js";
...
const serialize = (sale: SaleWithRelations) => ({
  ...
  attachments: sale.attachments.map((row) => attachmentDto("sales", sale.id, row)),
});

export function calculateSaleTotals(input: Pick<SaleInput, "amount" | "items" | "discountType" | "discountValue" | "taxRate" | "manualTotalOverride" | "paymentStatus" | "amountPaid">) {
  const subtotal = roundMoney(input.items?.reduce((sum, item) => sum + item.lineTotal, 0) ?? input.amount);
  const discountAmount = roundMoney(input.discountType === "PERCENTAGE" ? subtotal * (input.discountValue / 100) : input.discountType === "FIXED" ? Math.min(input.discountValue, subtotal) : 0);
  const taxable = roundMoney(Math.max(0, subtotal - discountAmount));
  const taxAmount = roundMoney(taxable * (input.taxRate / 100));
  const amount = roundMoney(input.manualTotalOverride ? input.amount : taxable + taxAmount);
  const requestedPaid = input.paymentStatus === "PAID" ? amount : input.paymentStatus === "UNPAID" ? 0 : input.amountPaid ?? 0;
  return { subtotal, discountAmount, taxAmount, amount, ...paymentState(amount, requestedPaid) };
}
...
const stockLines = (items: Array<{ catalogItemId?: string | null; measurement?: number | Prisma.Decimal | null }>) =>
  items.map((item) => ({ catalogItemId: item.catalogItemId, quantity: item.measurement == null ? 1 : Number(item.measurement) }));

async function reconcileSaleStock(tx: Prisma.TransactionClient, previous: ReturnType<typeof stockLines>, next: ReturnType<typeof stockLines>, reference: string, notes: string) {
  for (const { catalogItemId, delta } of stockDeltas(previous, next)) {
    const item = await tx.catalogItem.findUniqueOrThrow({ where: { id: catalogItemId } });
    if (item.kind !== "PRODUCT") continue;
    const beforeQty = Number(item.quantity);
    const type = delta > 0 ? "STOCK_IN" as const : "STOCK_OUT" as const;
    const quantity = Math.abs(delta);
    const afterQty = stockAfterMovement(beforeQty, type, quantity);
    await tx.catalogItem.update({ where: { id: item.id }, data: { quantity: afterQty } });
    await tx.inventoryMovement.create({ data: { catalogItemId, type, quantity, beforeQty, afterQty, reference, notes } });
  }
}
...
export async function createSale(input: SaleInput) {
  ...
  const sale = await prisma.$transaction(async (tx) => {
    const created = await tx.sale.create({ data: {
      ...
      paymentStatus: totals.status, amountPaid: totals.amountPaid, balanceDue: totals.balanceDue,
    ...
    await reconcileSaleStock(tx, [], stockLines(items), reference, "Automatically deducted from sale");
    if (totals.amountPaid > 0) await tx.payment.create({ data: { amount: totals.amountPaid, method: input.paymentMethod, direction: "IN", saleId: created.id, customerId: created.customerId, reference, notes: "Opening sale payment" } });
    return created;
  });
...
export async function updateSale(id: string, input: SaleUpdateInput) {
  ...
  const items = input.items ?? current.items.map((item) => ({ catalogItemId: item.catalogItemId, name: item.name, type: item.type, pricingMethod: item.pricingMethod, measurement: item.measurement === null ? null : Number(item.measurement), unit: item.unit, rate: Number(item.rate), lineTotal: Number(item.lineTotal), manualTotalOverride: item.manualTotalOverride }));
  const merged = {
  ...
  const totals = calculateSaleTotals(merged);
  if (totals.amountPaid < Number(current.amountPaid)) throw new Error("Recorded payments cannot be reduced");
  const customerRelation = input.customer?.id ? { connect: { id: input.customer.id } } : input.customer ? { create: { name: input.customer.name, phone: input.customer.phone || null, email: input.customer.email || null } } : undefined;
  const sale = await prisma.$transaction(async (tx) => {
    if (input.items) await reconcileSaleStock(tx, stockLines(current.items), stockLines(input.items), current.reference, "Reconciled after sale edit");
    ...
    const paymentDelta = roundMoney(totals.amountPaid - Number(current.amountPaid));
    if (paymentDelta > 0) await tx.payment.create({ data: { amount: paymentDelta, method: input.paymentMethod ?? current.paymentMethod, direction: "IN", saleId: current.id, customerId: updated.customerId, reference: updated.reference, notes: "Sale payment" } });
    return updated;
  });
  return serialize(sale);
}
...
export async function deleteSale(id: string) {
  const attachments = await prisma.$transaction(async (tx) => {
    const sale = await tx.sale.findUniqueOrThrow({ where: { id }, include: { items: true, attachments: true } });
  ...
  for (const row of attachments) {
    ...
    await getStorage().delete(row).catch((error) => console.error("Deleted sale attachment cleanup failed", error));
  }
}
