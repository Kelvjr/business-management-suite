...
async function main() {
  ...
  const supplierOne = await prisma.supplier.upsert({ where: { id: "supplier-beauty-depot" }, update: {}, create: { id:"supplier-beauty-depot",name:"Accra Beauty Depot",contactName:"Mavis Quaye",phone:"030 255 4401",email:"orders@beautydepot.example",paymentTerms:"30 days",outstandingBalance:1250 } });
  const supplierTwo = await prisma.supplier.upsert({ where: { id: "supplier-pro-hair" }, update: {}, create: { id:"supplier-pro-hair",name:"Pro Hair Ghana",contactName:"Daniel Ofori",phone:"024 811 0922",email:"sales@prohair.example",paymentTerms:"50% deposit",outstandingBalance:0 } });
