...
type SectionKey = "overview" | "sales" | "finance" | "customers" | "inventory" | "purchasing" | "marketing" | "reports" | "settings";
...
const sections: Section[] = [
  { key: "overview", label: "Overview", icon: Gauge, href: "/", items: [
    { label: "Dashboard", href: "/" }, { label: "Activity", href: "/activity" }, { label: "Alerts", href: "/alerts" },
  ] },
  { key: "sales", label: "Sales", icon: ShoppingBag, href: "/sales", items: [
    { label: "All Sales", href: "/sales" }, { label: "Record Sale", href: "/sales/new" }, { label: "Returns & Refunds", href: "/sales/returns" }, { label: "Sales Channels", href: "/sales/channels" },
  ] },
  { key: "finance", label: "Finance", icon: WalletCards, href: "/expenses", items: [
    { label: "Expenses", href: "/expenses" }, { label: "Invoices", href: "/invoices" }, { label: "Payments", href: "/finance/payments" }, { label: "Outstanding Balances", href: "/finance/outstanding" }, { label: "Transactions", href: "/finance/transactions" }, { label: "Taxes", href: "/finance/taxes" },
  ] },
  { key: "customers", label: "Customers", icon: Users, href: "/customers", items: [
    { label: "All Customers", href: "/customers" }, { label: "Customer Groups", href: "/customers/groups" }, { label: "Customer Insights", href: "/customers/insights" }, { label: "Notes & Activity", href: "/customers/activity" }, { label: "Leads", href: "/customers/leads" },
  ] },
  { key: "inventory", label: "Inventory", icon: Boxes, href: "/inventory", items: [
    { label: "Products & Services", href: "/catalog" }, { label: "Stock Levels", href: "/inventory" }, { label: "Stock Movements", href: "/inventory/movements" }, { label: "Low Stock", href: "/inventory/low-stock" }, { label: "Categories", href: "/inventory/categories" },
  ] },
  { key: "purchasing", label: "Purchasing", icon: PackageCheck, href: "/purchases", items: [
    { label: "Suppliers", href: "/suppliers" }, { label: "Purchases", href: "/purchases" }, { label: "Purchase History", href: "/purchasing/history" }, { label: "Supplier Balances", href: "/purchasing/balances" },
  ] },
  { key: "marketing", label: "Marketing", icon: Megaphone, href: "/marketing", items: [
    { label: "Campaigns", href: "/marketing#campaigns" }, { label: "WhatsApp Messages", href: "/marketing#messages" }, { label: "Reminders", href: "/marketing#reminders" }, { label: "Promotions", href: "/marketing#promotions" }, { label: "Automations", href: "/marketing#automations" },
  ] },
  { key: "reports", label: "Reports", icon: BarChart3, href: "/reports", items: [
    { label: "Sales Reports", href: "/reports/sales" }, { label: "Profit & Loss", href: "/reports/profit-loss" }, { label: "Expense Reports", href: "/reports/expenses" }, { label: "Customer Reports", href: "/reports/customers" }, { label: "Product Reports", href: "/reports/products" }, { label: "Inventory Reports", href: "/reports/inventory" }, { label: "Tax Reports", href: "/reports/taxes" },
  ] },
  { key: "settings", label: "Settings", icon: Settings, href: "/settings", items: [
    { label: "Business Profile", href: "/settings/business" }, { label: "Branding", href: "/settings/branding" }, { label: "Sales Settings", href: "/settings/sales" }, { label: "Finance Settings", href: "/settings/finance" }, { label: "Notifications", href: "/settings/notifications" }, { label: "Team & Access", href: "/settings/team" }, { label: "Integrations", href: "/settings/integrations" }, { label: "Help Centre", href: "/help" },
  ] },
];
...
function Rail({ selected, choose, close }: { selected: SectionKey; choose: (key: SectionKey) => void; close?: () => void }) {
  return <div className="flex h-full flex-col bg-[#f5f6ff] dark:bg-[#161B22]">
    ...
    <div className="grid place-items-center border-t border-violet-200/70 py-3 dark:border-white/8"><Link href="/help" title="Help Centre" className="grid size-10 place-items-center rounded-xl text-slate-500 hover:bg-white hover:text-violet-700 dark:text-slate-400 dark:hover:bg-white/8 dark:hover:text-white"><CircleHelp className="size-[18px]" /></Link></div>
  </div>;
}
  ...
  return <div className="flex h-full min-w-0 flex-col bg-white dark:bg-[#161B22]">
    ...
    <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-5" aria-label={`${section.label} menu`}>
      ...
      <div className="space-y-1">{section.items.map((item) => {
        const active = item.href === pathname || (section.key === "marketing" && pathname === "/marketing" && item.label === "Campaigns");
    ...
    <div className="p-3">
      <div className="rounded-xl border bg-white p-3 shadow-sm dark:border-white/8 dark:bg-white/[.035]">
        <div className="flex items-center gap-2"><Sparkles className="size-4 text-violet-600"/><p className="text-[11px] font-semibold">Your workspace is connected</p></div>
        <p className="mt-2 text-[10px] leading-4 text-muted-foreground">Sales, finance, customers, stock, marketing, and reports now work as one system.</p>
        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-violet-100 dark:bg-white/8"><div className="h-full w-full rounded-full bg-violet-600"/></div>
      </div>
      <Link href="/help" onClick={close} className="mt-2 flex h-9 items-center gap-2 rounded-lg px-3 text-[11px] text-muted-foreground hover:bg-muted hover:text-foreground"><LifeBuoy className="size-4"/>Help Centre</Link>
    </div>
  </div>;
}
...
export function AppShell({ title, subtitle, children, actions, mobileNavOpen, onMobileNavChange }: AppShellProps) {
  ...
  return <div className="app-canvas min-h-screen lg:grid lg:grid-cols-[64px_208px_minmax(0,1fr)]">
    <aside className="sticky top-0 hidden h-screen border-r border-violet-200/70 lg:block dark:border-white/8"><Rail selected={selectedKey} choose={() => undefined}/></aside>
    ...
    <main className="relative min-w-0">
      <header className="sticky top-0 z-30 border-b bg-background/95 backdrop-blur-2xl">
        <div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:px-7">
          <div className="flex min-w-0 items-center gap-3"><Button variant="outline" size="icon" className="shrink-0 lg:hidden" onClick={() => onMobileNavChange(true)}><Menu/></Button><h1 className="truncate font-display text-[17px] font-bold tracking-[-.03em] sm:text-xl">{title}</h1></div>
